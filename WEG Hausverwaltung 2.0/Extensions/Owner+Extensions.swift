...
extension Owner {
    ...
    var heatersArray: [Heater] {
        (heaters as? Set<Heater>)?.sorted { $0.room.localizedCaseInsensitiveCompare($1.room) == .orderedAscending } ?? []
    }
    
    /// Zugriff auf die Mieter als Array (sortiert nach Nachname)
    var tenantsArray: [Tenant] {
        (tenants as? Set<Tenant>)?.sorted { $0.lastName.localizedCaseInsensitiveCompare($1.lastName) == .orderedAscending } ?? []
    }
    ...
    static var examples: [Owner] {
        ...
        func createOwner(firstName: String, lastName: String, apt: String, floor: String, share: Double, occupants: Int16, area: Double) -> Owner {
        ...
        return [
            createOwner(firstName: "Max", lastName: "Mustermann", apt: "A102", floor: "1", share: 16.67, occupants: 2, area: 75.0),
            createOwner(firstName: "Anna", lastName: "Schmidt", apt: "B201", floor: "2", share: 18.33, occupants: 3, area: 85.0)
        ]
    }
