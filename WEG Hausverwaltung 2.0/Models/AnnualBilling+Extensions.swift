...
extension AnnualBilling {
    ...
    var statusText: String {
        return isFinalized ? "Abgeschlossen" : "In Bearbeitung"
    }
    
    /// Status-Farbe für UI
    var statusColor: String {
        return isFinalized ? "green" : "orange"
    }
    ...
    var computedTotalWaterCost: Double {
        return apartmentBillingsArray.reduce(0) { $0 + $1.waterCost }
    }
    
    /// Berechnet die gesamten Gaskosten
    var computedTotalGasCost: Double {
        return apartmentBillingsArray.reduce(0) { $0 + $1.gasCost }
    }
    
    /// Berechnet die gesamten Heizkosten
    var computedTotalHeatingCost: Double {
        return apartmentBillingsArray.reduce(0) { $0 + $1.heatingCost }
    }
    
    /// Berechnet die Gesamtkosten aller Kostenarten
    func getTotalCost() -> Double {
        return totalWaterCost + totalGasCost
    }
    ...
    func validateBillings(in context: NSManagedObjectContext) -> Bool {
        ...
        do {
            ...
            let billingOwners = apartmentBillingsArray.compactMap { $0.owner }
