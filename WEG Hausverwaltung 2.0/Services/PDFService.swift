...
final class PDFService {
    ...
    enum PDFError: LocalizedError {
        ...
        var errorDescription: String? {
            ...
            case .generationFailed(let reason):
                return "PDF-Generierung fehlgeschlagen: \(reason)"
            case .invalidData:
                return "Ungültige Daten für PDF"
            case .lockFailed(let year):
                return "PDF für Jahr \(year) konnte nicht gesperrt werden"
            case .fileNotFound(let path):
                return "PDF nicht gefunden: \(path)"
            }
        }
    }
    ...
    private func generatePDFData(for owner: Owner, year: Int) async throws -> Data {
        // Implementierung der PDF-Generierung
        throw PDFError.generationFailed("Noch nicht implementiert")
    }
    
    private func getPDFPath(for year: Int) -> String? {
        // Implementierung der Pfadermittlung
        return nil
    }
    
    private func drawBillingPDF(for owner: AppOwner, in context: UIGraphicsPDFRendererContext) {
